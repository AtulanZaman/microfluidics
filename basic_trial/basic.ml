...
type bin_op =PLUS|MINUS|MULT|DIV|MOD|EQUAL|LESS|LESSEQ|GREAT|GREATEQ|DIFF|AND|OR;;
type expression =
ExpInt of int
|ExpVar of string 
|ExpStr of string
...
let pp_binop = function PLUS ->"+" | MULT -> "*"| MOD ->"%" | MINUS ->"-"| DIV ->"/" | EQUAL ->" = " | LESS ->" < "| LESSEQ ->" <= "| GREAT->" > "| GREATEQ ->" >= "| DIFF ->" <> "| AND->" & " | OR ->" |";;

let pp_unrop = function UMINUS ->"-"|NOT->"!";;

let parenthesis x = "(" ^ x ^ ")" ;;

let pp_expression =let rec ppl pr = function
ExpInt n -> (string_of_int n)
| ExpVar v -> v
| ExpStr s -> "\"" ^ s ^ "\""
| ExpUnr (op,e) -> let res = (pp_unrop op)^(ppl (priority_uop op) e)
in if pr=0 then res else parenthesis res
| ExpBin (e1,op,e2)->let pr2 = priority_binop op
in let res = (ppl pr2 e1)^(pp_binop op)^(ppr pr2 e2)
in if pr2 >= pr then res else parenthesis res and ppr pr exp =
match exp with 
ExpBin (e1,op,e2)->
...
let extract pred cl = let st = cl.string and pos = cl.current in
let rec ext n = if n<cl.size && (pred st.[n]) then ext (n+1) else n in
let res = ext pos
in cl.current <- res;String.sub cl.string pos (res-pos);;


let extract_int = let is_int = function '0'..'9'-> true |_->false
in function cl ->int_of_string (extract is_int cl)
let extract_ident = let is_alpha_num = function 'a'..'z' | 'A'..'Z' | '0'..'9' | '_'-> true
| _ ->false
...
| '"'->forward cl ;
       ...
       in forward cl ; res | '+' | '-' | '*' | '/' | '%' | '&' | '|' | '!' | '=' | '('| ')' -> forward cl; Lsymbol (String.make 1 c)
| '<'
| '>'->forward cl;
...
let reduce pr = function
    (Texp e) :: (Tunr op) :: st when (priority_uop op) >= pr->(Texp (ExpUnr (op,e))) :: st
| (Texp e1) :: (Tbin op) :: (Texp e2) :: st when (priority_binop op) >= pr-> (Texp (ExpBin (e2,op,e1))) :: st
| _ -> raise ParseError;;
...
| Lsymbol s , _ -> let symbol = if s<>"-" then tsymb s
                                else match stack
                                with (Texp _)::_ ->Tbin MINUS
                                | _-> Tunr UMINUS
in ( match symbol with
            Tunr op -> (Tunr op) :: stack
            |Tbin op-> ( try stack_or_reduce lex (reduce (priority_binop op) stack )
...
let parse_exp stop cl = let p = ref 0 in
...
|"GOTO"-> Goto (match lexer cl with Lint p -> p
                                    | _ -> raise ParseError)
|"INPUT"-> Input (match lexer cl with Lident v-> v
                                    | _ ->raise ParseError)
| "PRINT" ->Print (parse_exp ((=) Lend) cl)
| "LET" -> let l2 = lexer cl and l3 = lexer cl
in ( match l2 ,l3 with (Lident v,Lsymbol "=") ->  Let (v,parse_exp ((=) Lend) cl)
| _ ->raise ParseError )
| "IF"-> let test = parse_exp ((=) (Lident "THEN")) cl
...
let parse str = let cl = init_lex str
